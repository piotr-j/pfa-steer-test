 ...
 */
public class CustomLinePath<T extends Vector<T>> implements Path<T, CustomLinePath.LinePathParam> {
	...
	public CustomLinePath (Array<T> waypoints) {
	 ...
	 * @throws IllegalArgumentException if {@code waypoints} is {@code null} or has less than two (2) waypoints. */
	public CustomLinePath (Array<T> waypoints, boolean isOpen) {
		this.isOpen = isOpen;
	...
	/** A {@code LinePathParam} contains the status of a {@link CustomLinePath}.
	 *
	 * @author davebaol */
	...
	/** A {@code Segment} connects two consecutive waypoints of a {@link CustomLinePath}.
	 *
	...
	public static class Segment<T extends Vector<T>> {
		...
		/** Returns the cumulative length from the first waypoint of the {@link CustomLinePath} this segment belongs to. */
